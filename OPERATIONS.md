...
**Goal:** prove collect → validate → summarize → judge → browse all work on a
tiny corpus before scaling. Do this first, every time you change the pipeline.

1. **Collect corpus** — pulls the 10 smoke texts → `smoke/texts/`.
2. **Validate corpus** — runs the 9 checks. Must print **ALL CHECKS PASSED**.
3. **Summarize (r-grid)** — runs each configured summarizer across the r-grid
   → `smoke/summaries/<system>/...`.
4. **Judge summaries** — LLM-as-judge scores → `.judge.json` beside summaries.
5. **Browse results** — pick a summary → see source + summary + judge scores.
...
### 4. Judge summaries
**Judge pilot summaries** — scores the pilot summaries → `.judge.json`.

### 5. Browse
- **Browse texts** — all pilot texts with word count + rating status.
- **Browse results** — source + summary + judge scores (pilot dirs).
...
### 4. Judge summaries
**Judge main summaries** → `.judge.json` in the main summaries tree.
